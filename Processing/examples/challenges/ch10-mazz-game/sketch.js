...
function updatePlayerGraphics() {
  ...
  if (
    ...
    cells[playerPos].walls[0] == false
  ) {
    py -= 1;
  } else if (
    py < rows - 1 &&
    (keyIsDown(DOWN_ARROW) || keyIsDown(KEY_S)) &&
    cells[playerPos].walls[2] == false
  ) {
    py += 1;
  } else if (
    px < cols - 1 &&
    (keyIsDown(RIGHT_ARROW) || keyIsDown(KEY_D)) &&
    cells[playerPos].walls[1] == false
  ) {
    px += 1;
  } else if (
    px > 0 &&
    (keyIsDown(LEFT_ARROW) || keyIsDown(KEY_A)) &&
    cells[playerPos].walls[3] == false
  ) {
    px -= 1;
  }
...
/*function solveMaze(startIndex, endIndex) {
  var visitedCells = [];
  var c = cells[startIndex];
  
  var currentIndex = startIndex;
  visitedCells.push(startIndex);
  
  var visitNeighbors = [];
  if (c.walls[0] === false && visitedCells.indexOf(currentIndex - cols) === -1 && c.y > 0) {
    visitNeighbors.push(currentIndex - cols);
  } else if (c.walls[1] === false && visitedCells.indexOf(currentIndex + 1) === -1 && c.x < col - 1) {
    visitNeighbors.push(currentIndex + 1);
  } else if (c.walls[2] === false && visitedCells.indexOf(currentIndex + cols) === -1 && c.y < rows - 1) {
    visitNeighbors.push(currentIndex + cols);
  } else if (c.walls[3] === false && visitedCells.indexOf(currentIndex - 1)) === -1 && c.x > 0) {
    visitNeighbors.push(currentIndex - 1);
  }
  
  if (visitNeighbors.length === 0) {
