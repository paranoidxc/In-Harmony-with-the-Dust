...
class Worker
{
  ...
  public function __construct(?string $socketName = null, array $socketContext = [])
  {
    ...
    // 使用 spl_object_hash() 函数为每个 Worker 实例生成唯一的哈希值作为 ID
    // 这个 ID 用于在全局静态数组中识别不同的 Worker 实例
    $this->workerId = spl_object_hash($this);
    // 创建一个空的标准类对象，用于存储 Worker 实例的自定义数据
    $this->context = new stdClass();

    // 将当前 Worker 实例存储到类的静态属性 $workers 数组中
    // 这确保了即使在函数作用域外，Worker 实例也不会被垃圾回收
    static::$workers[$this->workerId] = $this;

    // 为当前 Worker 实例创建一个空数组，用于后续存储所有子进程的 PID
    ...
    // 设置一个空的默认 onMessage 回调函数
    // 这样即使用户没有定义处理消息的回调，也不会导致错误
  ...
  {
    // runAll() 是 Workerman 框架的核心方法，负责启动整个服务，
    // 将所有注册的 Worker 实例运行起来
    try {
      // 检查运行环境是否为 CLI 模式
      static::checkSapiEnv();
      static::initStdOut();
      // 初始化全局状态和配置
      //  - 设置进程状态为 STATUS_STARTING
      //  - 注册 autoload 函数
      //  - 设置错误处理函数
      //  - 获取启动文件路径
      //  - 初始化全局计时器
      static::init();
      // 解析命令行参数 处理 start, stop, restart, reload, status 等命令
      static::parseCommand();
      // 检查端口是否可用
  ...
  protected static function init(): void
  {
    ...
    put_log([
      ...
      "statusFile" => static::$statusFile,
      "statisticsFile" => static::$statisticsFile,
      "connectionsFile" => static::$connectionsFile,
    ...
    put_log([
      "tip" => "[main] ----- after call initGlobalEvent",
      "globalEvent" => static::$globalEvent,
    ]);
    ...
    put_log([
      "tip" => "[main] ----- before Init data for worker id",
      "count_workers" => count(static::$workers),
      "idMap" =>  static::$idMap,
    ]);
    // Init data for worker id.
    static::initId();
    put_log([
      "tip" => "[main] ----- after Init data for worker id",
      "count_workers" => count(static::$workers),
      "idMap" =>  static::$idMap,
    ]);
  ...
  protected static function initWorkers(): void
  {
    ...
    //var_dump('static::$workers', static::$workers);
    foreach (static::$workers as $worker) {
      ...
      put_log([
        "tip" => "[main] ----- initWorker 初始化 process 下的主worker",
      ...
      // 记录 Worker 监听的套接字地址
      // 为状态显示和日志记录提供信息
      // 将套接字名称（如 tcp://0.0.0.0:8080）保存到 context 对象中
      ...
      if (!$worker->reusePort) {
        put_log([
          "tip" => "[main] ----- initWorker info",
          "reusePort" => $worker->reusePort,
          "listen" => "called",
        ]);

        // listen方法负责创建套接字并绑定到指定的地址和端口，开始监听连接请求。
        $worker->listen();
        // pauseAccept 方法暂停接受新的连接请求，通常在主进程初始化子进程前使用。
        $worker->pauseAccept();
      }
    }
  }
  ...
  protected static function forkWorkersForLinux(): void
  {
    foreach (static::$workers as $worker) {
      ...
      put_log([
        "tip" => "[worker] ----- before fork",
        'pidMap' => static::$pidMap,
      ]);
  ...
  protected static function forkOneWorkerForLinux(self $worker): void
  {
    ...
    elseif (0 === $pid) {
      ...
      if (static::$globalEvent === null) {
        static::$eventLoopClass = $worker->eventLoop ?: static::$eventLoopClass;
        put_log([
          "tips" => "[thread] ----- globalEvent",
          "globalEvent" => static::$eventLoopClass
        ]);
        static::$globalEvent = new static::$eventLoopClass();
        static::$globalEvent->setErrorHandler(function ($exception) {
          static::stopAll(250, $exception);
      ...
      put_log([
        "tip" => "[thread] ----- now run(), globalEvent->run()",
      ]);
      $worker->run();
      // Main loop.
      static::$globalEvent->run();
  ...
  protected static function writeStatisticsToStatusFile(): void
  {
    ...
    if (static::$masterPid === posix_getpid()) {
      ...
      file_put_contents(static::$statisticsFile, 'start time:' . date(
        'Y-m-d H:i:s',
      ...
      file_put_contents(
        static::$statisticsFile,
        "pid\tmemory  " . str_pad('listening', static::getUiColumnLength('maxSocketNameLength')) . " " . str_pad(
          'name',
          static::getUiColumnLength('maxWorkerNameLength')
  ...
  public function listen(): void
  {
    put_log([
      "tip" => "[listen] ----- call",
      "scokerName" => $this->socketName,
    ]);

    if (!$this->socketName) {
      put_log([
        "tip" => "[listen] ----- not has socketName",
      ]);
      return;
    }

    put_log([
      "tip" => "[listen] ----- has socketName",
    ]);

    // 所有的work 
    if (!$this->mainSocket) {
      put_log([
        "tip" => "[worker/listen] ----- create mainSocket",
      ]);
      ...
      // 创建服务器套接字
      put_log([
        "tip" => "[worker] ----- create socker server",
  ...
  public function pauseAccept(): void
  {
    if (
      ...
      put_log(["tip" => "[worker] ----- pauseAccept set true"]);
  ...
  public function resumeAccept(): void
  {
    ...
    if (static::$globalEvent !== null && $this->pauseAccept === true && $this->mainSocket !== null) {
      ...
      put_log(["tips" => "[worker] ----- resumeAccept not real call"]);
    }
  }
  ...
  {
    put_log([
      "tip" => "[thread] ----- run listen",
      "socketName"    => $this->socketName,
      //"ainSocket" => $this->mainSocket,
    ]);
    $this->listen();
    ...
    put_log(["tip" => "[thread] ----- has onWorkerStart callback"]);

    // Try to emit onWorkerStart callback.
    $callback = function () {
      try {
        put_log(["tip" => "[thread] ----- callback called , now call onWorkerStart(this)"]);
    ...
    put_log([
      "tip" => "[thread] ----- eventLoopClass type",
      "eventLoopClass" => Worker::$eventLoopClass
    ]);
    ...
    switch (Worker::$eventLoopClass) {
      ...
      default:
        put_log([
          "tip" => "[thread] ----- Fiber(callback)->start()",
        ]);
  ...
  protected function acceptTcpConnection(mixed $socket): void
  {
    ...
    // Thundering herd.
    // 惊群处理机制, .TODO. 后面的代码还是会执行，
    // 但是 tcp 的 baseRead 函数还是会走到底
    if (!$newSocket) {
      return;
    }
    ...
    if ($this->onConnect) {
      ...
      put_log(["tips" => "---------- not call onConnect ------------;"]);
    }
  }
