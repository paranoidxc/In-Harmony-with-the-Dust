...
// FBN generates fractal brownian motion
func Fbm2(x, y, frequency, lacunaryity, gain float32, octaves int) float32 {
	var sum float32
	amplitude := float32(1.0)

	for i := 0; i < octaves; i++ {
		sum += Snoise2(x*frequency, y*frequency) * amplitude
		frequency = frequency * lacunaryity
...
func MakeNoise(noiseType NoiseType, frequency, lacunarity, gain float32, octaves int, w, h int) (noise []float32, min float32, max float32) {
