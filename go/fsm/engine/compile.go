...
func Compile(cmdExprs []CmdExpr) (*StateMachine, error) {
	...
	xsm := &StateMachine{
		numStates:   uint64(len(cmdExprs) + 1),
...
func compileCmdExprsToNFA(cmdExprs []CmdExpr) *nfaNode {
	...
	for _, child := range cmdNfaNodes {
		fmt.Printf("cmdNfaNodes: %+v\n", child)
		t := nfaEmptyTransition{nextNode: child}
		root.emptyTransitions = append(root.emptyTransitions, t)
	}

	fmt.Printf("-------- root: %+v\n", root)
	if len(root.emptyTransitions) > 0 {
		for idx, ev := range root.emptyTransitions {
			fmt.Printf("-------- emptyTransitions[%d].: %+v\n", idx, ev.nextNode)
		}
	}

	if len(root.eventTransitions) > 0 {
		for idx, ev := range root.eventTransitions {
			fmt.Printf("--------- eventTransitions[%d].: %+v\n", idx, ev.nextNode)
		}
	}
	return root
}
