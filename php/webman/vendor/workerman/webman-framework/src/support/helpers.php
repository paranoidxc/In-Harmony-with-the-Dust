...
if (!function_exists('worker_bind')) {
  ...
  {
    put_log(["tips" =>  "-------- worker_bind", 'worker' => $worker, 'class' => $class]);
    $callbackMap = [
      'onConnect',
      'onMessage',
      'onClose',
    ...
    if (method_exists($class, 'onWorkerStart')) {
      put_log(["tips" =>  "-------- instance onWorkerStart call"]);
...
if (!function_exists('worker_start')) {
  ...
  {
    put_log([
      "tips" => "[main] ----- process worker_start",
      'pocessName' => $processName,
      'config' => $config,
    ]);
    ...
    // 当创建 Worker 实例时，Workerman 会自动将其添加到 Worker::$workers 静态数组中保存，
    // 例如：static::$workers[$this->workerId] = $this;
    ...
    // 定义 onWorkerStart 回调，加载应用基础组件并绑定处理类
    // 这个回调函数被赋值给 Worker 实例，
    // 会在 worker 进程启动时被调用，这发生在 Worker::runAll() 执行之后。
    $worker->onWorkerStart = function ($worker) use ($config) {
      put_log(["tips" => "[worker] ----- worker->onWorkerStart"]);
      require_once base_path('/support/bootstrap.php');
      if (isset($config['handler'])) {
        put_log([
          "tips" => "[worker] ----- worker handler",
...
if (!function_exists('put_log')) {
  ...
  {
    $dest = debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS, 0);
    $trace = [];
    $cnt = 0;
    for ($i = count($dest) - 1; $i >= 0; $i--) {
      $tmp = $dest[$i];
      $func = $tmp['function'] ?? "";

      if ($func) {
        $file = $tmp['file'] ?? "";
        $file = str_replace("/app/vendor/workerman/", "~/", $file);
        $file = str_replace("/app/vendor/webman/", "~/", $file);
        $tr = [];
        $tr['file'] = $file;
        $tr['func'] = $func;
        $tr['line'] = $tmp['line'] ?? "";
    ...
    $data_str = json_encode(array_merge(
      [
        'dt' => date("Y-m-d H:i:s"),
        'pid' => posix_getpid(),
      ],
    ...
    file_put_contents("./xxx.txt", $data_str . "\n -------------------****************-------------------\n", FILE_APPEND);
  }
}
