...
class Particle {
  ...
  show() {
    ...
    if (drawSettings.drawVectors) {
      push();
      translate(this.position.x, this.position.y);

      const scaledVelocity = this.velocity.copy();
      scaledVelocity.mult(2);

      noFill();
      strokeWeight(4);
      stroke(drawSettings.useFill ? color(255) : this.color);
      line(0, 0, scaledVelocity.x, scaledVelocity.y);
