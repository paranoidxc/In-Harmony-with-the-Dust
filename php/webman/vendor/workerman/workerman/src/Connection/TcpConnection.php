...
class TcpConnection extends ConnectionInterface implements JsonSerializable
{
    ...
    public const TCP_KEEPALIVE_INTERVAL = 1;
    ...
    public function __construct(EventInterface $eventLoop, $socket, string $remoteAddress = '')
    {
        ...
        $remote_details = $remoteAddress . '_' . microtime(true);
    ...
    public function baseRead($socket, bool $checkEof = true): void
    {
        ...
        $buffer = '';
        try {
            // 使用 fread 从套接字读取数据，最大读取 READ_BUFFER_SIZE 字节
            // 读取失败或读到空数据时检查连接是否已关闭，如果是则销毁连接
        ...
        if ($buffer === '' || $buffer === false) {
            ...
            if ($this->recvBuffer === '') {
                // 如果接收缓冲区为空，尝试使用请求缓存机制
                // 对于常见的请求模式，使用静态数组 $requests 缓存请求对象以提高性能
                if (!isset($buffer[static::MAX_CACHE_STRING_LENGTH]) && isset($requests[$buffer])) {
                    ...
                    if ($this->protocol === Http::class) {
                        ...
                        try {
                            put_log([
                                'tip' => "[tcp] baseRead onMessageCall-1",
                            ]);
                    ...
                    try {
                        put_log([
                            'tip' => "[tcp] baseRead onMessageCall-2",
                        ]);
                        ($this->onMessage)($this, $request);
                    } catch (Throwable $e) {
                        $this->error($e);
        ...
        put_log([
            'tip' => "[tcp] ----- baseRead process protocol",
            'protocol' => $this->protocol,
        ]);
        // If the application layer protocol has been set up.
        if ($this->protocol !== null) {
            // 如果设置了应用层协议（$this->protocol），进入协议解析循环
            // 使用当前包长度 currentPackageLength 判断数据是否足够组成完整包
            // 如果包长度未知，调用 protocol::input() 获取包长度
            // 处理各种异常情况：包过大、数据不足、协议错误等
            //put_log(['tip' => "[tcp] -> [protocol] - 1"]);
            while ($this->recvBuffer !== '' && !$this->isPaused) {
                // The current packet length is known.
                put_log([
                    'tip' => "[tcp] -> [protocol] -> [recvBuffer]",
                    'currentPackageLength' => $this->currentPackageLength,
                ]);
                if ($this->currentPackageLength) {
                    // Data is not enough for a package.
                    if ($this->currentPackageLength > strlen($this->recvBuffer)) {
                        //put_log(['tip' => "tcpConnection baseRead 000000------- 1"]);
                        break;
                    }
                } else {
                    ...
                    try {
                        put_log([
                            'tip' => "[tcp] -> [protocol] -> 2 call http::input",
                            "msg" => "如果包长度未知，调用 protocol::input() 获取包长度",
                        ]);
                        $this->currentPackageLength = $this->protocol::input($this->recvBuffer, $this);
                        //put_log(['tip' => "tcpConnection baseRead 000000------- 2"]);
                    } catch (Throwable $e) {
                        //put_log(['tip' => "tcpConnection baseRead 000000------- 3"]);
                        $this->currentPackageLength = -1;
                        Worker::safeEcho((string)$e);
                    }
                    // The packet length is unknown.
                    if ($this->currentPackageLength === 0) {
                        //put_log(['tip' => "tcpConnection baseRead 000000------- 4"]);
                        break;
                    } elseif ($this->currentPackageLength > 0 && $this->currentPackageLength <= $this->maxPackageSize) {
                        // Data is not enough for a package.
                        // 当套接字可读时，调用baseRead方法读取数据
                        // 如果数据不足以组成完整包，将保留已读取的数据在recvBuffer中
                        // 退出当前方法，等待下次可读事件触发再继续读取
                        // 当累积足够数据后，才会完成一个完整包的处理
                        if ($this->currentPackageLength > strlen($this->recvBuffer)) {
                            put_log(['tip' => "[tcp]-> [protocol] -> 包数据不够"]);
                            break;
                        }
                    } // Wrong package.
                    else {
                        //put_log(['tip' => "tcpConnection baseRead 000000------- 6"]);
                    ...
                    //put_log(['tip' => "tcpConnection baseRead 000000------- 6-1"]);
                }
                ...
                //put_log(['tip' => "tcpConnection baseRead 000000------- 6-2"]);
                try {
                    // Decode request buffer before Emitting onMessage callback.
                    put_log([
                        'tip' => "[tcp] -> [protocal] -> [decode]",
                        'oneRequestBuffer' => $oneRequestBuffer,
                    ]);
                    $request = $this->protocol::decode($oneRequestBuffer, $this);
                    //put_log(['tip' => "tcpConnection baseRead 000000------- 6-3"]);
                    if ((!is_object($request) || $request instanceof Request) && $one && !isset($oneRequestBuffer[static::MAX_CACHE_STRING_LENGTH])) {

                        put_log(['tip' => "[tcp] -> [protocal] -> onMessage -> 1222"]);
                        //var_dump($request);
                        ($this->onMessage)($this, $request);
                        ...
                        //var_dump($request);
                        //put_log(['tip' => "tcpConnection baseRead 000000------- 7"]);
                        return;
                    }
                    put_log(['tip' => "tcpConnection read ok call onMessage-3"]);
                    ($this->onMessage)($this, $request);
                } catch (Throwable $e) {
                    //put_log(['tip' => "tcpConnection baseRead 000000------- 6-4"]);
                    $this->error($e);
                }
            }
            put_log(['tip' => "[tcp] -> [protocal] - okay end while recv return"]);
            return;
        }
        //put_log(['tip' => "tcpConnection baseRead 3333333333333333333 "]);
        ...
        ++self::$statistics['total_request'];
        try {
            put_log(['tip' => "[tcp] ----- no protocal read ok call onMessage-4"]);
