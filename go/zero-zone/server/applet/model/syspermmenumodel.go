...
type (
	...
	SysPermMenuModel interface {
		...
		FindOneByName(ctx context.Context, name string) (*SysPermMenu, error)
	}
...
func (m *customSysPermMenuModel) FindOneByName(ctx context.Context, name string) (*SysPermMenu, error) {
	resp := SysPermMenu{}
	query := fmt.Sprintf("select %s from %s where `name` = ? limit 1", sysPermMenuRows, m.table)
