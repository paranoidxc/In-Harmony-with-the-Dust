	...
	for row := 0; row < len(levelMap); row++ {
		for col := 0; col < len(levelMap[row]); col++ {
			...
			if cell == 'T' {
				...
				player = players.New(tileSize/2, tileSize, x, y)
			}
		}
	}
...
func (l *level) handlerVerticalCollisoin() {
	l.applyGravity()
	...
	if (l.player.OnGround && l.player.Direction.Y < 0) ||
		l.player.Direction.Y > 1 {
