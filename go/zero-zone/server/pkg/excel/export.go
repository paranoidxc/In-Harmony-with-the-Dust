...
func ExportExcel(sheet, title, fields string, isGhbj, isIgnore bool, list interface{}, changeHead map[string]string, e *model.Excel) (err error) {
	...
	if err != nil {
		fmt.Printf("normalBuildTitle err %+v", err)
		return
	}
	// 构造数据行
	err = normalBuildDataRow(e, sheet, endColName, fields, dataRow, isGhbj, isIgnore, dataValue)
	if err != nil {
		fmt.Printf("normalBuildDataRow err %+v", err)
	}
	return
}

// ================================= 通用导出 =================================
func CustomerDynamicExport(sheet, title, fields string, isOddEvenRowStyle, isIgnoreFields bool, header interface{}, list interface{}, changeHead map[string]string) (file *excelize.File, err error) {
	if header != nil {
		headerVal := reflect.ValueOf(header)
		if headerVal.Kind() != reflect.Slice {
			return nil, errors.New("无效的数据类型")
		}
		e, _, endColName, dataRow, err := buildCustomHeader(header, sheet, title)

		dataValue := reflect.ValueOf(list)
		// 构造数据行
		err = normalBuildDataRow(e, sheet, endColName, "", dataRow, isOddEvenRowStyle, false, dataValue)
		ySplit := dataRow

		if len(title) > 0 {
			ySplit -= 1
		}
		activeCell := fmt.Sprintf("A%d", ySplit+1)
		// 冻结表头
		e.F.SetPanes(sheet, &excelize.Panes{
			Freeze:      true,
			YSplit:      ySplit,
			TopLeftCell: activeCell,
			ActivePane:  "bottomLeft",
			Selection: []excelize.Selection{
				{SQRef: activeCell, ActiveCell: activeCell, Pane: "bottomLeft"},
			},
		})
		//{SQRef: "K16", ActiveCell: "K16", Pane: "topRight"},
		// 合并表头单元格
		//HorizontalMerge(e.F, sheet, 1, dataRow)
		// 纵向合并数据行内容
		//VerticalMerge(e.F, sheet, 1, nil)

		return e.F, err
	} else {
		e := model.ExcelInit()
		err = ExportExcel(sheet, title, fields, isOddEvenRowStyle, isIgnoreFields, list, changeHead, e)
...
func NormalDynamicExport(sheet, title, fields string, isGhbj, isIgnore bool, list interface{}, changeHead map[string]string) (file *excelize.File, err error) {
	...
	e.F.SetPanes("Sheet1", &excelize.Panes{
		Freeze:      true,
		YSplit:      2,
		TopLeftCell: "A34",
		ActivePane:  "bottomLeft",
...
func normalBuildDataRow(e *model.Excel, sheet, endColName, fields string, row int, isGhbj, isIgnore bool, dataValue reflect.Value) (err error) {
	...
	for i := 0; i < dataValue.Len(); i++ {
		...
		for j := 0; j < num; j++ {
			...
			//
			fontColorVal := item.FieldByName("Fontcolor") // 取字段值
			if dataCol.Fontcolor != "" && fontColorVal.String() == "1" {
				//if dataCol.Fontcolor != "" {
				//fmt.Println(dataCol.Fontcolor)
				//fmt.Println(dataField.Name)
				//fmt.Println(fontColorVal.String())
				//e.F.SetCellStyle(sheet, startCol, endCol, e.ContentStyleTmp)
				//e.F.SetCellStyle(sheet, startCol, endCol, e.ContentStyleTmp)
				// 调用回调函数
				/*
					callbackFunc := getCallbackFunction(funcName)
					if callbackFunc != nil {
						// 调用回调函数并打印结果
						result := callbackFunc()
						fmt.Println("Callback function result:", result)
					}

					if dataCol.Ical == "MyTest" {
					}
				*/
			}
		...
		rowStyle := model.ODD_ROW
		if isGhbj && row%2 == 0 {
			rowStyle = model.EVEN_ROW
		}

		fgFontColorVal := item.FieldByName("FgFontColor") // 取字段值
		if fgFontColorVal.Kind() == reflect.Int {
			if fgFontColorVal.Int() == model.FG_RED_ROW {
				rowStyle = rowStyle | model.FG_RED_ROW
			}
		}
		e.F.SetCellStyle(sheet, startCol, endCol, e.RowContentStyle(rowStyle))
