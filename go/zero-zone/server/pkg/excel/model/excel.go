...
const (
	ExcelTagKey = "excel"
	Pattern     = "name:(.*?);|fontcolor:(.*?);|index:(.*?);|width:(.*?);|replace:(.*?);"
)
...
func (e *ExcelTag) setValue(tag []string, value string) {
	...
	if strings.Contains(tag[0], "fontcolor") {
		e.Fontcolor = value
	}
...
type Excel struct {
	...
	ContentStyleTmp int            // 主体样式2，有背景色
}
...
func (e *Excel) getDataRowFontStyle() {
	style := excelize.Style{}
	style.Font = &excelize.Font{ // 背景颜色
		Color: "#FF0000",
	}
	e.ContentStyleTmp, _ = e.F.NewStyle(&style)
}
...
func (e *Excel) RowContentStyle(rowStyleType int) int {
	style := excelize.Style{}
	style.Border = getBorder()
	style.Alignment = &excelize.Alignment{
		Horizontal: "center",
	...
	if rowStyleType&FG_RED_ROW == FG_RED_ROW {
		style.Font = &excelize.Font{
			Size:  12,
			Color: "FF0000",
		}
	...
	if rowStyleType&EVEN_ROW == EVEN_ROW {
		style.Fill = excelize.Fill{ // 背景颜色
			Type:    "pattern",
			Color:   []string{"#cce7f5"},
			Pattern: 1,
	...
	ContentStyle, _ := e.F.NewStyle(&style)
	return ContentStyle
}
