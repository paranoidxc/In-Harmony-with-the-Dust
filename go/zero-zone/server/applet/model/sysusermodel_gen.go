...
var (
	...
	cacheZoneZoneAdminSysUserIdPrefix      = "cache:zeroZone:sysUser:id:"
	cacheZoneZoneAdminSysUserAccountPrefix = "cache:zeroZone:sysUser:account:"
)
...
func (m *defaultSysUserModel) Delete(ctx context.Context, id int64) error {
	...
	arkAdminSysUserIdKey := fmt.Sprintf("%s%v", cacheZoneZoneAdminSysUserIdPrefix, id)
...
func (m *defaultSysUserModel) FindOne(ctx context.Context, id int64) (*SysUser, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, arkAdminSysUserIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
		query := fmt.Sprintf("select %s from %s where `id` = ? AND deleted_at IS NULL limit 1", sysUserRows, m.table)
...
func (m *defaultSysUserModel) FindOneByAccount(ctx context.Context, account string) (*SysUser, error) {
	arkAdminSysUserAccountKey := fmt.Sprintf("%s%v", cacheZoneZoneAdminSysUserAccountPrefix, account)
	var resp SysUser
	err := m.QueryRowIndexCtx(ctx, &resp, arkAdminSysUserAccountKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) (i interface{}, e error) {
		query := fmt.Sprintf("select %s from %s where `account` = ? AND deleted_at IS NULL limit 1", sysUserRows, m.table)
...
func (m *defaultSysUserModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheZoneZoneAdminSysUserIdPrefix, primary)
}
